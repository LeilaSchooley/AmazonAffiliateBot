...
def create_amazon_affiliate_description(title, keyword):
    generated_text = None
    prompt = f"Generate a text to promote the product {title} using the text {keyword}."
    message = {
        "role": "user",
        "content": prompt
    }
...
def create_video_with_subtitles(video_script_path, background_video_path, background_music_path, output_path):
    ...
    clip = CompositeVideoClip([clip, subtitle_source.set_pos(('center', "bottom"))]).speedx(factor=1.1).subclip(
        0, 60)
...
create_video_with_subtitles('../data/video_script.txt', 'output.mp4',
